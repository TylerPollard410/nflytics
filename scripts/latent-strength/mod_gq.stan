...
generated quantities {
  ...
  {
    vector[N_teams] ts_cur = filtered_team_strength;
    real league_hfa_cur = filtered_league_hfa;
    vector[N_teams] team_hfa_cur = filtered_team_hfa;
  ...
  for (k in 1:N_oos) {
    ...
    if (w <= N_weeks) {
      ...
      if (is_last_week[last_w] == 1) {
        ts_target = phi_season_team_strength_innovation * filtered_team_strength;
      } else {
        ts_target = phi_weekly_team_strength_innovation * filtered_team_strength;
      }
      hfa_home = filtered_team_hfa[i];
    }
