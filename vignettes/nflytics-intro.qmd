...
`posterior_trajectory()` mirrors the `posterior_trajectory()` interface from
`rstanarm`, summarising latent team strength over time. By default it returns
credible intervals per team-week combination.

```{r trajectory, eval=FALSE}
trajectory_summary <- posterior_trajectory(
  fit_cmdstan,
  summary = TRUE,
  probs = c(0.1, 0.9)
)
...
Set `summary = FALSE` to obtain one row per posterior draw, which is useful
for plotting ribbons with `tidybayes` or `ggplot2`.
...
Once you have either the summary data or the raw draws, you can create tidy
visualisations with `ggplot2`. The chunk below assumes you executed the
`posterior_trajectory()` code above and have `trajectory_summary` available.
...
If you prefer to visualise individual draws, combine
`posterior_trajectory(summary = FALSE)` with `tidybayes::stat_lineribbon()`
after filtering to the team(s) of interest.
